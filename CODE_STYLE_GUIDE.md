...
8. `bool` vars should be named as questions, e.g. `is_set` instead of `set`. Functions returning `bool` should be worded as questions with a verb prefix if they would otherwise conflict with a field or variable, e.g. `check_is_set` or `get_is_set`.
